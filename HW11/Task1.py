        ...
        with psycopg2.connect(**config) as conn:
            with conn.cursor() as cur:
                ...
                if choice == '1': 
                    ...
                    cur.execute("""
                                CREATE OR REPLACE PROCEDURE insert_or_update(name_input VARCHAR(255), number_input VARCHAR(255))
                                AS $$
                                BEGIN
                    ...
                    cur.execute("""
                                CREATE OR REPLACE PROCEDURE insert_many_data(name_input VARCHAR(255)[], number_input VARCHAR(255)[])
                                AS $$       
                                DECLARE
                                    i INT;
                                    incorrect_data TEXT[] := '{}';
                                ...
                                BEGIN
                                    FOR i IN 1..ARRAY_LENGTH(name_input, 1) LOOP
                                        IF number_input[i] !~ '^\d{1,255}$' THEN
                ...
                with open(file_path, newline='', encoding='utf-8') as f:
                    ...
                    for row in reader:
                        if len(row) == 2:
                            name, number = row
                            cur.execute("""
                                        CREATE OR REPLACE PROCEDURE insert_by_CSV(name_input VARCHAR(255), number_input VARCHAR(255))
                                        AS $$
                                        DECLARE
                                            unique_data TEXT[] := '{}';
                                        BEGIN
                                            ...
                                            ELSE
                                                INSERT INTO PhoneBook(name, number) VALUES (name_input, number_input);
                                            END iF;
                                            RAISE NOTICE 'Skipped Data (duplicates) --> %s', unique_data;
                                        END;
                                        $$ LANGUAGE plpgsql;
                                        """)
        ...
        with psycopg2.connect(**config) as conn:
            with conn.cursor() as cur:
                ...
                if choice == '1':
                    ...
                    cur.execute("""
                                CREATE OR REPLACE PROCEDURE update_by_number(new_number VARCHAR(225), identifier VARCHAR(255))
                                AS $$
                                BEGIN
            ...
            with conn.cursor() as cur:
                ...
                if choice == '1':
                    cur.execute("""
                                ...
                                RETURNS TABLE(id INT, name VARCHAR(255), number VARCHAR(255))
                                AS $$
                                BEGIN
                                    RETURN QUERY
                    ...
                    cur.execute("""
                                CREATE OR REPLACE FUNCTION search_by_number(exact_number VARCHAR(255))
                                RETURNS TABLE(id INT, name VARCHAR(255), number VARCHAR(255))
                                AS $$
                                BEGIN
                                    RETURN QUERY
        ...
        with psycopg2.connect(**config) as conn:
            with conn.cursor() as cur:
                ...
                if choice == '1':
                    ...
                    cur.execute("""
                                CREATE OR REPLACE PROCEDURE delete_by_number(number_input VARCHAR(225))
                                AS $$
                                BEGIN
